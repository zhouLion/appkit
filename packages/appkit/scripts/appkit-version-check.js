...
/* CONFIG ------------------------------------------------------------------ */
const REOWN_SCOPE_PREFIX = '@cotofe/appkit'
  ...
  while (current !== dirname(current)) {
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        // Check if this package.json has @cotofe/appkit dependencies
        ...
        // If it has @cotofe/appkit deps and it's not the @cotofe/appkit package itself
        if (hasReownDeps && pkg.name !== '@cotofe/appkit') {
...
function getReownPackagesFromProject(project) {
  // Get ALL @cotofe/appkit packages from the project
...
function reportResults(installed, baseline, mismatched) {
  console.log('Installed @cotofe/appkit packages and versions:')
  ...
  if (mismatched.length > 0) {
    ...
    console.error(`   Expected all @cotofe/appkit packages to be version ${baseline}\n`)
    console.error('   Mismatched packages:')
    mismatched.forEach(([pkg, version]) => {
      console.error(`     • ${pkg}: ${version} (expected ${baseline})`)
    })
    console.error(`\n   Please update all @cotofe/appkit packages to version ${baseline}`)
    ...
    console.log(`\u001b[32m✓ All @cotofe/appkit packages are in sync (${baseline})\u001b[0m`)
    return true
  }
}
...
function main() {
  ...
  if (!project) {
    console.log('Could not find consumer project with @cotofe/appkit dependencies')
    return
  }
  ...
  console.log('Found @cotofe/appkit packages:', Object.keys(allReownPackages))

  if (!Object.keys(allReownPackages).length) {
    console.log('No @cotofe/appkit packages found in project')
    return
  }
  ...
  if (!Object.keys(installed).length) {
    console.log('No @cotofe/appkit packages could be resolved')
    return
  }
